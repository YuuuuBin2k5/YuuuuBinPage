...
const SkillsShowcase = () => {
  ...
  const techStack = [
    ...
    { name: "Next.js", icon: "▲", color: "from-slate-200 to-slate-400", category: "frontend" },
    { name: "TypeScript", icon: "TS", color: "from-blue-500 to-blue-600", category: "frontend" },
    { name: "Tailwind", icon: "🎨", color: "from-cyan-400 to-blue-400", category: "frontend" },
    { name: "Vue", icon: "V", color: "from-green-400 to-emerald-500", category: "frontend" },
    { name: "Redux", icon: "🔄", color: "from-purple-500 to-purple-600", category: "frontend" },
    
    // Backend Row 2
    { name: "Node.js", icon: "📗", color: "from-green-500 to-green-600", category: "backend" },
    { name: "Java", icon: "☕", color: "from-red-500 to-orange-500", category: "backend" },
    { name: "Spring", icon: "🍃", color: "from-emerald-500 to-green-500", category: "backend" },
    { name: "Python", icon: "🐍", color: "from-blue-500 to-yellow-500", category: "backend" },
    { name: "Express", icon: "⚡", color: "from-slate-400 to-slate-600", category: "backend" },
    { name: "PHP", icon: "🐘", color: "from-indigo-500 to-purple-500", category: "backend" },
    
    // Database Row 3
    { name: "MySQL", icon: "🐬", color: "from-blue-500 to-blue-600", category: "database" },
    { name: "PostgreSQL", icon: "🐘", color: "from-blue-600 to-indigo-600", category: "database" },
    { name: "MongoDB", icon: "🍃", color: "from-green-600 to-emerald-600", category: "database" },
    { name: "Redis", icon: "🔴", color: "from-red-500 to-red-600", category: "database" },
    { name: "Firebase", icon: "🔥", color: "from-yellow-500 to-orange-500", category: "database" },
    ...
    { name: "AWS", icon: "☁️", color: "from-orange-400 to-yellow-500", category: "devops" },
    { name: "Linux", icon: "🐧", color: "from-slate-600 to-slate-700", category: "devops" },
    { name: "VS Code", icon: "💻", color: "from-blue-500 to-cyan-500", category: "devops" },
    { name: "Figma", icon: "🎨", color: "from-purple-500 to-pink-500", category: "devops" },
  ];
  ...
  const getAnimationClass = (index) => {
    const col = index % 6;
    if (col < 2) return "animate-[flyInFromLeft_0.8s_cubic-bezier(0.34,1.56,0.64,1)_forwards]";
    if (col >= 4) return "animate-[flyInFromRight_0.8s_cubic-bezier(0.34,1.56,0.64,1)_forwards]";
    if (index < 6) return "animate-[flyInFromTop_0.8s_cubic-bezier(0.34,1.56,0.64,1)_forwards]";
    ...
    <div className="relative py-20 px-6 overflow-hidden bg-slate-900">
      ...
      <div className="relative z-10 w-full px-6">
            ...
            {techStack.map((tech, index) => (
              ...
              >
                ...
                <div className="relative aspect-square p-4 bg-slate-800/60 backdrop-blur-sm rounded-2xl border border-white/10 overflow-hidden transition-all duration-500 hover:scale-110 hover:border-white/30 hover:-translate-y-2 hover:shadow-2xl hover:shadow-black/50 cursor-pointer group-hover:animate-[flipIn_0.6s_ease-in-out]">
                  ...
                  {/* Glow Effect */}
                  <div className={`absolute inset-0 bg-gradient-to-br ${tech.color} opacity-0 group-hover:opacity-30 blur-2xl transition-opacity duration-500`} />
                  
                  {/* Shine Effect on Hover */}
                  <div className="absolute inset-0 bg-gradient-to-br from-white/0 via-white/20 to-white/0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 transform -translate-x-full group-hover:translate-x-full" style={{ transition: "transform 0.6s ease-in-out, opacity 0.3s" }} />
        ...
        <div className="mt-16 grid grid-cols-2 md:grid-cols-4 gap-6 max-w-4xl mx-auto">
          {[
            { label: "Frontend", count: techStack.filter(t => t.category === "frontend").length, icon: "🎨" },
            { label: "Backend", count: techStack.filter(t => t.category === "backend").length, icon: "⚙️" },
            { label: "Database", count: techStack.filter(t => t.category === "database").length, icon: "💾" },
            { label: "DevOps", count: techStack.filter(t => t.category === "devops").length, icon: "🚀" },
          ].map((stat, index) => (
            <div
              key={index}
